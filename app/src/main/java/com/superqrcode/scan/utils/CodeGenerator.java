...
public class CodeGenerator extends AsyncTask<Void, Void, Bitmap> {
    ...
    public static void setColor(int BLACK) {
    ...
    protected Bitmap doInBackground(Void... voids) {
        try {
            if (TYPE == TYPE_QR) {
                ...
                return createBarcode(this.input, barcodeFormat);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private Bitmap createBarcode(String data, BarcodeFormat barcodeFormat) throws WriterException {
        MultiFormatWriter writer = new MultiFormatWriter();
        String finalData = Uri.encode(data);

        // Use 1 as the height of the matrix as this is a 1D Barcode.
        BitMatrix bm = writer.encode(finalData, barcodeFormat, BAR_WIDTH, 1);
